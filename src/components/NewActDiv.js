...
export default function NewActDiv({
  ...
  const toggleIsCompleted = () =>
    setInput({ ...input, isComplete: !input.isComplete });
  ...
  return (
    <div className="bg-white absolute opacity-100 left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 mx-auto max-w-md px-10 py-5 space-y-5 border border-gray-300 z-10">
      <div>
        {error && <div>{error}</div>}
        <form onSubmit={(e) => onFormSubmit(e)} className="flex flex-col gap-4">
          ...
          <div className="flex justify-center">
            <button
              type="submit"
              className="border border-gray-400 p-2 rounded-md w-fit"
            >
              Submit
            </button>
          </div>
      ...
      <div className="absolute w-[27px] h-[27px] right-6 top-0 border rounded-full flex justify-center items-center cursor-pointer hover:bg-gray-300 hover:text-white transition duration-300">
        <button
          ...
          onClick={() => setShowCreateAct(false)}
        >
          x
        </button>
      </div>
